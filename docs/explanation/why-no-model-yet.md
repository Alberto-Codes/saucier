...
point at. A regular expression recovers 124 sauce preparations and links 29 of
them to a base.

That number is the bar. Any model applied to this source has to beat 29, and
...
95 where the source states none. Those outputs pass validation, read
correctly, and are wrong.

The parser cannot produce that failure. It resolves a parent only where the
opening paragraph names exactly one, so `parent` is `None` on 95 entries.
Unresolved is a visible, countable state. Confidently wrong is not.

## The unresolved entries are the finding

95 of 124 preparations do not state a base in their opening paragraph. That
