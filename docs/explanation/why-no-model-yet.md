...
That number is the bar. Any model applied to this source has to beat 64, and
until a parser has been run, nobody knows what "beating it" would mean. A
model that recovers 70 looks impressive in isolation and is nearly worthless
next to free.
...
102 where the source states none. Those outputs pass validation, read
correctly, and are wrong.

The parser cannot produce that failure. It resolves a parent only where the
opening paragraph names one, so `parent` is `None` on 102 entries. Unresolved
is a visible, countable state. Confidently wrong is not.

## The unresolved entries are the finding

102 of 166 preparations do not state a base in their opening paragraph. That
