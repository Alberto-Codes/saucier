"""Banned-term gate: the rejected project name stays out of the tree.

The project was named `shortorder` before it was named `saucier`. That name
promised speed, and the destination is a slow compositional sauce, so it was
rejected. A rename without a gate drifts back, because docs get amended and
notes get appended.

This gate fails on any occurrence of the rejected name outside `ALLOWLIST`.
The match ignores case, so it catches title-case prose too.

Each allowlist entry names a file whose subject *is* the rejected name. An
entry matching nothing fails as well: a stale entry widens the gate without
saying so, which is the drift this gate exists to prevent.
...
BANNED_TERM = "shortorder"

ALLOWLIST: dict[str, str] = {
    "scripts/check_banned_terms.py": "This gate defines the term.",
    "docs/reference/glossary.md": (
        "The glossary records the rejected name so the rejection is legible "
